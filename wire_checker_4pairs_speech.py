...
def create_audio_files():
    """Create simple audio files if they don't exist"""
    try:
        # Create a simple beep sound for "GOOD"
        if not os.path.exists(AUDIO_GOOD):
            # Create a simple sine wave for "GOOD" (higher frequency)
            import numpy as np
            sample_rate = 44100
            duration = 0.5
            frequency = 800
            ...
            import wave
            with wave.open(AUDIO_GOOD, 'w') as wav_file:
                wav_file.setnchannels(1)
                wav_file.setsampwidth(2)
                wav_file.setframerate(sample_rate)
                wav_file.writeframes(audio_data.tobytes())
            print(f"Created {AUDIO_GOOD}")
        
        # Create a simple beep sound for "NOT GOOD" (lower frequency)
        if not os.path.exists(AUDIO_NOT_GOOD):
            import numpy as np
            sample_rate = 44100
            duration = 0.8
            frequency = 400
            ...
            import wave
            with wave.open(AUDIO_NOT_GOOD, 'w') as wav_file:
                wav_file.setnchannels(1)
                wav_file.setsampwidth(2)
                wav_file.setframerate(sample_rate)
                wav_file.writeframes(audio_data.tobytes())
            print(f"Created {AUDIO_NOT_GOOD}")
            
    except Exception as e:
        print(f"Error creating audio files: {e}")
        # Fallback to buzzer if audio creation fails
        return False
    return True
    ...
    while True:
        try:
            ...
            for i, (output_pin, input_pin) in enumerate(WIRE_PAIRS):
                ...
                pair_status[i] = is_connected  # Update global pair status
            ...
            with status_lock:
                if has_cross_connections:
                    ...
                    solenoid2_control(True)  # Turn on solenoid2
                else:
                    current_status = "OPEN"
...
class WireCheckerUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Wire Checker Status - 4 Pairs (Speech)")
        ...
        if cycle_id:
            ...
            self.cycle_label.pack(pady=(0, 25))
...
def main():
    ...
    audio_created = create_audio_files()
    if not audio_created:
        print("Warning: Could not create audio files. Using buzzer fallback.")
